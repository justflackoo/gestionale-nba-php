...
if(!empty($data->nome_ruolo)){
  $ruolo->nome_ruolo = $data->nome_ruolo;

          if($ruolo->create()){
              http_response_code(201); //Setto il codice di risposta 201 --> CREATO
              echo json_encode(array("message" => "Ruolo creato con successo."));//E lo comunico all'utente

          }else{ 

              http_response_code(503);//Entro in questo blocco se non sono riuscito a creare la canotta  
              echo json_encode(array("message" => "Non è stato possibile creare un nuovo ruolo."));} //E lo comunico all'utente :(

}else{
        //Se l'utente non ha inserito tutti i dati entriamo in questo else
      http_response_code(400); 
      echo json_encode(array("message" => "Dati incompleti. Impossibile creare il ruolo."));}
