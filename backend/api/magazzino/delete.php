...
if(!empty($data->id_giacenza)){
        //Se l'ID c'è, lo assegno all'oggetto che ho creato alla riga 15
        $magazzino->id_giacenza = $data->id_giacenza; 

        //Adesso provo ad eliminare la giacenza a cui corrisponde l'id che ho passato
        if($magazzino->delete()) {   
                //Eliminazione è andata a buon fine
                http_response_code(200); // 200 OK
                echo json_encode(array("message" => "Giacenza rimossa con successo."));
        } else { 
                // Entro qui se c'è stato un problema lato server/database (es. query errata)
                http_response_code(503);  // 503 Service Unavailable
                echo json_encode(array("message" => "Non è stato possibile rimuovere la giacenza."));
        }
      ...
      //Entro in questo else se il client si è dimenticato di inviare l'id o lo ha lasciato vuoto
        ...
        echo json_encode(array("message" => "Dati incompleti. Ricordati di inviare l'id_giacenza per confermare l'eliminazione."));
        }
